...
class model_classification(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,embedding_dim, hidden_size):
        ...
        with tf.name_scope('weights'):
            self.weights = {
                ...
                'softmax_ours': tf.Variable(tf.random_uniform([6 * self.hidden_size, self.class_num], -0.01, 0.01)),
                'softmax_uv':tf.Variable(tf.random_uniform([2 * self.hidden_size, self.class_num],-0.01,0.01)),
        ...
        with tf.name_scope('biases'):
            self.biases = {
                ...
                'constants':tf.constant(np.ones(self.class_num),tf.float32),
                ...
                'softmax_uv':tf.Variable(tf.random_uniform([self.class_num],-0.01,0.01)),
                'softmax_ours':tf.Variable(tf.random_uniform([self.class_num],-0.01,0.01)),
            }

        with tf.name_scope('embedding'):
            self.word_embedding = tf.constant( self.embedding_file , name="word_embedding", dtype=tf.float32)
            # self.word_embedding = tf.Variable(self.embedding_file, name="word_embedding", dtype=tf.float32)
    ...
    def user_attention(self):
        ...
        with tf.name_scope('u_word_attention'):
            ...
            alpha = tf.reshape(tf.matmul(u, self.weights['u_v_1']),
                               [batch_size, 1, self.max_sen_len])#batch_matmul
            alpha = self.softmax(alpha, self.sen_len, self.max_sen_len)
            outputs = tf.matmul(alpha, outputs)#batch_matmul
        ...
        with tf.name_scope('u_sentence_attention'):
            ...
            alpha = tf.reshape(tf.matmul(u, self.weights['u_v_2']),
                               [batch_size, 1, self.max_doc_len])#batch_matmul
    ...
    def softmax_my(self,arr):
        return tf.exp(arr)/(tf.reduce_sum(tf.exp(arr))+1e-10)
    def build_model(self):
        ...
        with tf.name_scope('softmax'):
            out1 =  self.u_doc
            out2 =  self.p_doc
            outputs = tf.concat([out1,out2],1)
            self.scores =tf.add( tf.matmul(outputs, self.weights['softmax']), self.biases['softmax'],name="score")
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = 0.6*tf.reduce_mean(losses) + 0.2*self.u_loss  + 0.2*self.p_loss #+ 0.4* self.uv_loss

            params = ['u_v_1','u_v_2','u_wh_1','u_wh_2','p_v_1','p_v_2','p_wh_1','p_wh_2','softmax']
            _regulars=0.0
            # for item in params:
                # _regulars += tf.reduce_sum(tf.square(self.weights[item]))
            # self.loss+= 0* _regulars

        with tf.name_scope("metrics"):
            correct_predictions = tf.equal(self.predictions, tf.argmax(self.input_y, 1))
            self.correct_num = tf.reduce_sum(tf.cast(correct_predictions, dtype=tf.int32), name="correct_num")
            self.y_true = tf.argmax(self.input_y,1,"true")
